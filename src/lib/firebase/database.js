...
export const getExpensesByMonth = async (uid, year, month) => {
  ...
  try {
    ...
    if (err.code === "failed-precondition") {
      console.warn("Firestore index needed. Visit Firebase console to create composite index for expenses collection.");
    } else {
      console.error("getExpensesByMonth failed:", err);
    }
    return [];
  }
};
...
export const getTodayExpenses = async (uid) => {
  ...
  try {
    ...
    console.error("getTodayExpenses failed:", err);
    return [];
  }
};
...
export const getSavPotByMonth = async (uid, year, month) => {
  ...
  try {
    ...
    if (err.code === "failed-precondition") {
      console.warn("Firestore index needed for savpotLedger collection.");
    } else {
      console.error("getSavPotByMonth failed:", err);
    }
    return [];
  }
};
...
export const getDailySnapshots = async (uid, year, month) => {
  try {
    ...
    console.error("getDailySnapshots failed:", err);
    return [];
  }
};
...
export const getExpensesByDate = async (uid, date) => {
  ...
  try {
    ...
    console.error("getExpensesByDate failed:", err);
    return [];
  }
};
...
// Uses a lock to prevent duplicate transfers from multiple tabs
export const checkAndRunMissedTransfers = async (uid) => {
  const config = await getSetupConfig(uid);
  if (!config) return;

  // Prevent duplicate transfers using a simple localStorage lock
  const lockKey = `savpot_transfer_lock_${uid}`;
  const lockExpiry = localStorage.getItem(lockKey);
  if (lockExpiry && Date.now() < Number(lockExpiry)) {
    return; // Another tab is handling transfers
  }
  localStorage.setItem(lockKey, String(Date.now() + 30000)); // 30s lock
